...
const OperationsInput = () => {
	...
	let handleResolveClick = () => {
		...
		if (isBlockGood && isValidChar) {
			...
			operations.map(operation => {
				let [result, newVars] = analizer(operation, variables);
				variables = newVars
				formattedResults.push(`Resultado de ${operations[operationNum]}: ${result[0]}`);
				operationNum++;
			})
	...
	return (
		<div className='operations-input'>
			...
			<p className='operations-input-instruction'>3. Puedes asignar variables con un identificador</p>
			<p className='operations-input-instruction'>4. Puedes usar identificadores en operaciones</p>
			<p className='operations-input-instruction'>5. Solo se pueden ingresar numeros enteros positivos</p>
			<textarea
